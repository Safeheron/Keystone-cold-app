...
import com.keystone.cold.protobuf.EosTxProtoc;
...
public class ChooseWatchWalletFragment extends ListPreferenceFragment {
      ...
      case MPC:
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
          @Override public void run() {
            ...
            MPCWalletEntity wallets = dao.loadWallet();
            AppExecutors.getInstance().mainThread().execute(() -> {
              if (wallets == null || TextUtils.isEmpty(wallets.address)) {
                navigate(R.id.action_to_createMpcWalletGuide);
              } else  {
                updateCurrentWatchWallet();
                stepIntoMainActivity();
              }
  ...
  @Override protected int getEntries() {
    return R.array.watch_wallet_list;
  }

  @Override protected int getValues() {
  ...
  protected class Adapter
      extends BaseBindingAdapter<Pair<String, String>, SettingItemSelectableBinding> {
