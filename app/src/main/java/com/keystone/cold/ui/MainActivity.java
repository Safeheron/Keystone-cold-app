...
public class MainActivity extends FullScreenActivity {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    if (getIntent() != null) {
      currentFragmentIndex = getIntent().getIntExtra("currentFragmentIndex", R.id.drawer_wallet);
    }
    initViews();
    initNavController();
  ...
  private void initNavController() {
    ...
    mNavController.addOnDestinationChangedListener((controller, destination, arguments) -> {
      ...
      if (((watchWallet == WatchWallet.XRP_TOOLKIT
          ...
          || watchWallet == WatchWallet.MPC
          || watchWallet == WatchWallet.SOLANA) && label.equals(AssetFragment.TAG))) {
        index = 0;
      }
  ...
  private void initViews() {
    ...
    mBinding.drawer.addDrawerListener(new FullScreenDrawer.DrawerListenerAdapter() {
      @Override public void onDrawerSlide(@NonNull View drawerView, float slideOffset) {
        mBinding.drawer.getChildAt(0)
            .setX(mBinding.menuContainer.getWidth() + mBinding.menuContainer.getX());
      }
    });
  }
  ...
  public class DrawerClickListener implements DrawerAdapter.OnItemClickListener {
    ...
    @SuppressLint("NonConstantResourceId") @Override public void itemClick(int position) {
      ...
      switch (position) {
        case R.id.drawer_wallet:
          if (watchWallet == WatchWallet.MPC) {
            mNavController.navigate(R.id.assetFragment, null);
          } else if (watchWallet == WatchWallet.XRP_TOOLKIT
              || watchWallet == WatchWallet.METAMASK
              || watchWallet == WatchWallet.SOLANA) {
            NavOptions navOptions =
                new NavOptions.Builder().setPopUpTo(R.id.assetListFragment, false).build();
  ...
  @Override public void setSupportActionBar(@Nullable Toolbar toolbar) {
    ...
    if (!Utilities.hasUserClickPatternLock(this) || (supportFingerprint
        && !Utilities.hasUserClickFingerprint(this))) {
  ...
  private final List<Integer> securePages =
      Arrays.asList(R.id.fingerprintSettingFragment, R.id.fingerprintManageFragment,
          R.id.fingerprintEnrollFragment, R.id.fingerprintGuideFragment, R.id.setPasswordFragment,
  ...
  public void notice(String title, String subTitle, String buttonText, Runnable confirmAction) {
    ModalDialog dialog = new ModalDialog();
    CommonModalBinding binding =
        DataBindingUtil.inflate(LayoutInflater.from(this), R.layout.common_modal, null, false);
