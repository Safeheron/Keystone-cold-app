...
public class Utilities {
  ...
  public static final String IS_EXPORT_MPC_WALLET = "is_export_mpc_wallet";

  public static void alert(AppCompatActivity activity, @Nullable String title,
      @NonNull String message, String buttonText, Runnable action) {
    ModalDialog.showCommonModal(activity, title, message, buttonText, action);
  }
  ...
  public static void isExportMPCWallet(Context context, boolean isExport) {
    SharedPreferences sp = context.getSharedPreferences(SHARED_PREFERENCES_KEY, MODE_PRIVATE);
    sp.edit().putBoolean(IS_EXPORT_MPC_WALLET, isExport).apply();
  }

  public static boolean isExportMPCWallet(Context context) {
    SharedPreferences sp = context.getSharedPreferences(SHARED_PREFERENCES_KEY, MODE_PRIVATE);
    return sp.getBoolean(IS_EXPORT_MPC_WALLET, false);
  }
}
